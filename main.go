...
func main() {
	...
	container := widgets.NewQPlainTextEdit(nil)
	channel := webchannel.NewQWebChannel(nil)
	channel.RegisterObject("content", container)

	text := []byte("Hello world \n# 1st\n it is **cool** sample code\n")
	output := github_flavored_markdown.Markdown(text)
	content := fmt.Sprintf(`
	...
	container.SetPlainTextDefault(content)
	container.TextChanged()
	webpage.SetWebChannel(channel)

	webpage.SetHtml(getHTML(content), core.NewQUrl())
	webview.SetParent(wid)
